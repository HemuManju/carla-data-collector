...
def post_process_image(image, rotation, normalized=True, grayscale=True):
    """
    ...
    if normalized:
        image_range = np.max(image.astype(np.float32)) - np.min(
            image.astype(np.float32)
        )
        image_range = np.maximum(image_range, 1e-12)
        return (
            image.astype(np.float32) - np.min(image.astype(np.float32))
        ) / image_range
    else:
