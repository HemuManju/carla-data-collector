...
class GlobalRoutePlanner(object):
    ...
    def trace_route(self, origin, destination):
        ...
        for i in range(len(route) - 1):
            ...
            if edge['type'] != RoadOption.LANEFOLLOW and edge[
                    'type'] != RoadOption.VOID:
                ...
                for waypoint in path[closest_index:]:
                    ...
                    route_trace.append((current_waypoint, road_option))
                    if len(route
                           ) - i <= 2 and waypoint.transform.location.distance(
                               destination) < 2 * self._sampling_resolution:
                        break
                    elif len(
                            route
                    ) - i <= 2 and current_waypoint.road_id == destination_waypoint.road_id and current_waypoint.section_id == destination_waypoint.section_id and current_waypoint.lane_id == destination_waypoint.lane_id:
    ...
    def _build_topology(self):
        ...
        for segment in self._wmap.get_topology():
            ...
            seg_dict['entryxyz'], seg_dict['exitxyz'] = (x1, y1, z1), (x2, y2,
                                                                       z2)
            seg_dict['path'] = []
        ...
        for segment in self._topology:
            ...
            self._graph.add_edge(n1,
                                 ...
                                 entry_vector=np.array([
                                     ...
                                     entry_carla_vector.y, entry_carla_vector.z
                                 ]),
    ...
    def _localize(self, location):
        ...
        edge = None
        try:
            edge = self._road_id_to_edge[waypoint.road_id][
    ...
    def _successive_last_intersection_edge(self, index, route):
        ...
        for node1, node2 in [(route[i], route[i + 1])
            ...
            if candidate_edge[
                    'type'] == RoadOption.LANEFOLLOW and candidate_edge[
                        'intersection']:
                last_intersection_edge = candidate_edge
                last_node = node2
            else:
    ...
    def _turn_decision(self, index, route, threshold=math.radians(35)):
        ...
        if index > 0:
            if self._previous_decision != RoadOption.VOID \
                ...
                calculate_turn = current_edge[
                    'type'] == RoadOption.LANEFOLLOW and not current_edge[
                        'intersection'] and next_edge[
                            'type'] == RoadOption.LANEFOLLOW and next_edge[
                                'intersection']
                if calculate_turn:
                    ...
                    if tail_edge is not None:
                        next_edge = tail_edge
                    cv, nv = current_edge['exit_vector'], next_edge[
                        'exit_vector']
                    ...
                    deviation = math.acos(
                        np.clip(
                            np.dot(cv, nv) /
                            (np.linalg.norm(cv) * np.linalg.norm(nv)), -1.0,
                            1.0))
                    if not cross_list:
                        cross_list.append(0)
