...
class BehaviorAgent(BasicAgent):
    ...
    def collision_and_car_avoid_manager(self, waypoint):
        ...
        vehicle_list = [
            ...
            if dist(v) < 45 and v.id != self._vehicle.id
        ]
    ...
    def run_step(self, debug=False):
        ...
        if walker_state:
            ...
            distance = w_distance - max(
                walker.bounding_box.extent.y,
                walker.bounding_box.extent.x) - max(
        ...
        if vehicle_state:
            ...
            distance = distance - max(vehicle.bounding_box.extent.y,
                                      vehicle.bounding_box.extent.x) - max(
                                          self._vehicle.bounding_box.extent.y,
                                          self._vehicle.bounding_box.extent.x)
        ...
        # 3: Intersection behavior
                ...
                in [RoadOption.LEFT, RoadOption.RIGHT]):
            target_speed = min(
